...
fn test_pinned_memory_corruption_on_move() {
    ...
    unsafe {
        ...
        let new_fat_ptr = std::ptr::slice_from_raw_parts_mut(new_ptr as *mut (), metadata) as *mut PinnedStruct;
        ...
        assert!(
            current_slice_addr >= old_base_addr && current_slice_addr < old_base_addr + layout.size(),
            "The slice pointer should still point inside the old allocation! {current_slice_addr} vs {old_base_addr}"
        );
        
        assert!(
            current_slice_addr < new_base_addr || current_slice_addr >= new_base_addr + layout.size(),
